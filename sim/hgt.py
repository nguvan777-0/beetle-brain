...
def _crossover(pop, recipient_idx, donor_idx, rng):
    ...
    g_r = np.concatenate([wb_r, w1_r.reshape(n, -1), w2_r.reshape(n, -1)], axis=1)
    g_d = np.concatenate([wb_d, w1_d.reshape(n, -1), w2_d.reshape(n, -1)], axis=1)
    L   = g_r.shape[1]
    ...
    pop['W2'][recipient_idx]     = g_new[:, n_wb + n_w1:].reshape(w2_r.shape)
